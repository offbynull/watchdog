 ...
 */
package com.offbynull.watchdog.gradleplugin;
 ...
 * 
 *     dependencies {
 *         classpath group: 'com.offbynull.watchdog',  name: 'gradle-plugin',  version: 'PUT_CORRECT_VERSION_HERE'
 *     }
 * }
 * 
 * apply plugin: "java"
 ...
 * 
 * dependencies {
 *     compile group: 'com.offbynull.watchdog', name: 'user', version: 'PUT_CORRECT_VERSION_HERE'
 * }
 * </pre>
...
public class WatchdogPlugin implements Plugin<Project> {
    ...
    public void apply(Project target) {
        // Add config block
        WatchdogPluginConfiguration config = new WatchdogPluginConfiguration();
        target.getExtensions().add("watchdog", config);
    ...
    @SuppressWarnings("unchecked")
    private void addInstrumentActionToTask(String sourceType, Task task, WatchdogPluginConfiguration config) {
        task.doLast(x -> {
            try {
                ...
                throw new IllegalStateException("Watchdog instrumentation failed", e);
            }
        });        
    }
    
    private void instrument(File classesDir, Set<File> compileClasspath, WatchdogPluginConfiguration config) {
        try {
